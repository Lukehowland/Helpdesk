    ...
    <div class="card card-outline card-secondary collapsed-card">
        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                {{-- Filtro de actividad reciente eliminado - columna last_activity_at no existe en BD --}}
                <div class="col-md-6 text-right">
                    <label class="text-sm d-block">&nbsp;</label>
                    <button type="button" class="btn btn-default btn-sm" id="btnResetFilters"><i class="fas fa-eraser"></i>
                        Limpiar</button>
                    <button type="button" class="btn btn-primary btn-sm" id="btnRefresh"><i class="fas fa-sync-alt"></i>
                        Aplicar</button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title"><i class="fas fa-exclamation-triangle"></i> Cambiar Estado</h5>
                    <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>
                </div>
                <form id="changeStatusForm">
                    <div class="modal-body">
                        <div id="statusErrorAlert" class="alert alert-danger" style="display:none"><span
                                id="statusErrorMessage"></span></div>
                        <div class="callout callout-info"><i class="fas fa-info-circle"></i> Usuario: <strong
                                id="statusUserName">-</strong></div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label>Razón:</label>
                            <textarea id="statusReason" name="reason" class="form-control" rows="3" maxlength="500"
                                placeholder="Requerida si suspende..."></textarea>
                        </div>
                        <input type="hidden" id="statusUserId">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-dark" data-dismiss="modal"><i class="fas fa-times"></i>
                            Cancelar</button>
                        <button type="submit" id="btnConfirmStatus" class="btn btn-warning"><i class="fas fa-check"></i>
                            Confirmar</button>
                    </div>
                </form>
            </div>
    ...
    <div class="modal fade" id="deleteUserModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="callout callout-danger"><strong>¿Eliminar a <span id="deleteUserName">-</span>?</strong>
                    </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-dark" data-dismiss="modal"><i class="fas fa-times"></i>
                        Cancelar</button>
                    <button type="button" id="btnConfirmDelete" class="btn btn-danger"><i class="fas fa-trash"></i>
                        Eliminar</button>
                </div>
            </div>
        </div>
    ...
    <script>
        (function () {
            ...
            const UI = {
                ...
                renderTable() {
                    ...
                    tb.html(state.users.map(u => {
                        ...
                        // Show multiple roles with badges (max 2 visible + count for rest)
                        let roleBadges = 'Sin Rol';
                        if (u.roleContexts?.length) {
                            const roles = u.roleContexts.map(rc => Utils.getRoleDisplayName(rc.roleCode));
                            const uniqueRoles = [...new Set(roles)]; // Remove duplicates
                            if (uniqueRoles.length <= 2) {
                                roleBadges = uniqueRoles.map(r => `<span class="badge badge-info mr-1">${r}</span>`).join('');
                            } else {
                                // Show first 2 + count
                                const visible = uniqueRoles.slice(0, 2).map(r => `<span class="badge badge-info mr-1">${r}</span>`).join('');
                                const extra = uniqueRoles.length - 2;
                                roleBadges = visible + `<span class="badge badge-secondary" title="${uniqueRoles.slice(2).join(', ')}">+${extra}</span>`;
                            }
                        }
                        const ev = u.emailVerified ? '<span class="badge badge-success">Sí</span>' : '<span class="badge badge-secondary">No</span>';
                        return `<tr data-id="${u.id}"><td><code>${Utils.escapeHtml(u.userCode || 'N/A')}</code></td><td>${Utils.escapeHtml(u.email)}</td><td>${Utils.escapeHtml(name)}</td><td>${roleBadges}</td><td>${Utils.getStatusBadge(u.status)}</td><td>${ev}</td><td class="text-nowrap"><button class="btn btn-sm btn-primary btn-view" data-id="${u.id}" title="Ver"><i class="fas fa-eye"></i></button> <button class="btn btn-sm btn-indigo btn-roles" data-id="${u.id}" title="Roles"><i class="fas fa-shield-alt"></i></button> <button class="btn btn-sm btn-warning btn-status" data-id="${u.id}" title="Estado"><i class="fas fa-ban"></i></button> <button class="btn btn-sm btn-danger btn-delete" data-id="${u.id}" title="Eliminar"><i class="fas fa-trash"></i></button></td></tr>`;
            ...
            function initEvents() {
                ...
                $('#btnResetFilters').on('click', function () { state.filters = { status: 'active' }; state.currentPage = 1; $('#searchInput').val(''); $('#statusFilter').val('active'); $('#roleFilter,#emailVerifiedFilter,#companyFilter').val(''); $('#orderByFilter').val('created_at'); $('#orderDirFilter').val('desc'); UI.updateFilterSelection(); API.loadUsers(); });
